...
if (longTrade) {
  ...
  console.log(`   Stop Loss: $${longTrade.stopLoss.toFixed(2)} (${((longTrade.stopLoss / longTrade.entryPrice - 1) * 100).toFixed(2)}%)`);
  console.log(`   Take Profit: $${longTrade.takeProfit.toFixed(2)} (${((longTrade.takeProfit / longTrade.entryPrice - 1) * 100).toFixed(2)}%)`);
  ...
  if (slCorrect && tpCorrect) {
    ...
    if (!slCorrect) console.log('      ⚠️ Stop Loss يجب أن يكون تحت سعر الدخول!');
    if (!tpCorrect) console.log('      ⚠️ Take Profit يجب أن يكون فوق سعر الدخول!');
  }
} else {
...
if (shortTrade) {
  ...
  console.log(`   Stop Loss: $${shortTrade.stopLoss.toFixed(2)} (${((shortTrade.stopLoss / shortTrade.entryPrice - 1) * 100).toFixed(2)}%)`);
  console.log(`   Take Profit: $${shortTrade.takeProfit.toFixed(2)} (${((shortTrade.takeProfit / shortTrade.entryPrice - 1) * 100).toFixed(2)}%)`);
  ...
  if (slCorrect && tpCorrect) {
    ...
    if (!slCorrect) console.log('      ⚠️ Stop Loss يجب أن يكون فوق سعر الدخول!');
    if (!tpCorrect) console.log('      ⚠️ Take Profit يجب أن يكون تحت سعر الدخول!');
  }
} else {
...
console.log(`   السعر: $65,000 → Trailing SL: $${testTrade.trailingStopPrice.toFixed(2)}`);
...
console.log(`   السعر: $61,000 → Trailing SL: $${testTrade.trailingStopPrice.toFixed(2)}`);
console.log(`   يجب إغلاق؟ ${update2.shouldClose ? '✅ نعم (تحت SL)' : '❌ لا (خطأ!)'}`);

// السعر ينزل إلى 60000 (تحت الـ SL الجديد)
const update3 = tradeManager.updateTradeTrailing(testTrade, 60000, Date.now());
console.log(`   السعر: $60,000 → Trailing SL: $${testTrade.trailingStopPrice.toFixed(2)}`);
